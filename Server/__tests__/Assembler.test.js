...
describe('Assembler Unit Tests', () => {
    ...
    // Unit Testing #2: ensurePartsAvailable() throws error when there are insufficient parts to produce a single bike.
    ...
    // Unit Testing #3: ensurePartsAvailable() returns a result with insufficient_parts set as true when min_possible_assemblies is less than requested_amount.
    ...
    // Unit Testing #4: assembleBike() throws error when parameter requested_amount is set as a non-positive number
    test('throws error when parameter requested_amount is set as a non-positive number', async () => {
        const testBike = new Bike.Bikes({
            internalId:     'abc123',
            type:           'Mountain Bike',
            price:          400,
        ...
        const savedBike = await testBike.save();
        const result = await Assembler.assembleBike(savedBike.internalId, 7);
